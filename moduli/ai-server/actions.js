...
const ACTION_TOOL_DEFS = [
  ...
  },
  {
    ...
    description: "Propone di aprire una RICHIESTA AL SUPPORTO umano di RescueManager. NON crea nulla direttamente: mostra all'utente una scheda da rivedere/modificare e inviare. Usa SOLO dopo aver provato ad aiutare, quando: non sai/non puoi risolvere il problema, è un malfunzionamento dell'app (bug), oppure l'utente chiede esplicitamente di contattare l'assistenza/un operatore o di segnalare un problema. Se puoi risolvere tu, NON usarlo.",
    input_schema: {
      type: "object",
      properties: {
        subject: { type: "string", description: "Oggetto breve e chiaro del problema (max 200 caratteri)" },
        category: {
          type: "string",
          enum: ["domanda", "bug", "funzionalita", "fatturazione", "altro"],
          description: "bug=malfunzionamento, funzionalita=richiesta nuova funzione, fatturazione=pagamenti/abbonamento, domanda=dubbio d'uso, altro",
        },
        priority: {
          type: "string",
          enum: ["normal", "high", "urgent"],
          description: "urgent SOLO se blocca completamente il lavoro",
        },
        message: { type: "string", description: "Descrizione dettagliata: cosa stava facendo l'utente, cosa è successo, cosa ha già provato (riassumi dalla conversazione). Min 10 caratteri." },
      },
...
function buildProposal(name, input) {
  switch (name) {
    ...
    case "propose_support_ticket": {
      const catLabels = {
        domanda: "Domanda", bug: "Malfunzionamento", funzionalita: "Richiesta funzione",
        fatturazione: "Pagamenti/abbonamento", altro: "Altro",
      };
      const priLabels = { normal: "Normale", high: "Alta", urgent: "Urgente" };
      const category = ["domanda", "bug", "funzionalita", "fatturazione", "altro"].includes(input.category) ? input.category : "altro";
      const priority = ["normal", "high", "urgent"].includes(input.priority) ? input.priority : "normal";
      const msg = String(input.message || "");
      return {
        type: "action_proposal",
        action: "open_support_ticket",
        title: `Richiesta al supporto: ${input.subject || "nuovo problema"}`,
        details: [
          `🏷️ ${catLabels[category]}`,
          priority !== "normal" && `⚠️ Priorità ${priLabels[priority]}`,
          msg && `📝 ${msg.slice(0, 200)}${msg.length > 200 ? "…" : ""}`,
        ].filter(Boolean),
        payload: {
        ...
        confirm_label: "Contatta il supporto",
      };
    }
